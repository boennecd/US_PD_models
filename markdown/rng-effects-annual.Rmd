...
ctrl_default <- PF_control(
  N_fw_n_bw = 500L, N_smooth = 1000L, N_first = 1000L, n_max = 1000, 
...
log_n_eval({
  state_rng_size_inter <- PF_EM(
    ...
    model = "exponential", by = 12L, max_T = max(dat_con$tstop), 
    id = dat_con$gvkey_int, 
    Q_0 = diag(1, 2), type = "VAR", control = ctrl_default, 
    trace = 1L)
...
x <- signif(sapply(inter_ms, "[[", "fixed_effects"), 2)
x["(Intercept)", ]
# scale by standard deviations
sds <- apply(model.matrix(tail(inter_ms)[[1L]]$terms$fixed, dat_con), 2, sd)
sds["(Intercept)"] <- NA
round(x <- t(x * sds)           , 1)
...
x <- signif(sapply(size_inter_ms, "[[", "fixed_effects"), 2)
x["(Intercept)", ]
# scale by standard deviations
sds <- apply(
  model.matrix(tail(size_inter_ms)[[1L]]$terms$fixed, dat_con), 2, sd)
sds["(Intercept)"] <- NA
round(x <- t(x * sds)           , 1)
round(z <- t(t(x) - colMeans(x)), 1)
matplot(z, type = "l", lty = 1)
