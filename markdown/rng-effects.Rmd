...
take_xtra <- function(
  ...
  N_first = N_fw_n_bw, N_smooth_final = N_smooth, averaging_start = 100L, 
  eps = 1e-5){
  cl <- fit$call
  ctrl <- fit$control
...
```{r sd_rng, cache = 1}
get_approx_info_n_score <- function(fit, n_rep = 100){
  set.seed(26217670)
  replicate(n_rep, {
    rnorm(10000)
    fit$seed <- .Random.seed
    o <- PF_get_score_n_hess(fit, use_O_n_sq = FALSE)
...
```{r show_sd_rng}
state_rng_inter_derivs_final <- local({
  # look at score approximations
  scs <- sapply(state_rng_inter_derivs, "[[", "score")
  scs <- apply(scs, 1, function(x) cumsum(x) / seq_along(x))
  matplot(scs, type = "l")
  cat("Scores:\n")
  print(scs[nrow(scs), ])
  
  obs_info <- lapply(state_rng_inter_derivs, "[[", "obs_info")
  obs_info <- simplify2array(obs_info)
  obs_info <- lapply(seq_len(dim(obs_info)[3]), function(i){
    apply(obs_info[, , 1:i, drop = FALSE], 1:2, mean)
  })
  
  # check standard deviations as we add samples
  sds <- suppressWarnings(
    sapply(obs_info, function(x) sqrt(diag(solve(x)))))
  cat("Standard error estimates:\n")
  print(sds[, (ncol(sds) - 1):ncol(sds)], digits = 2)
  matplot(t(sds / sds[, ncol(sds)]), type = "l", col = "black")
  
  list(sds = sds, sd = sds[, ncol(sds)], obs_info = obs_info)
})

state_rng_inter_derivs_final$sd
sqrt(diag(vcov(base_fit)))
```
...
log_n_eval({
  state_rng_size_restrict_xtra <- take_xtra(
    state_rng_size_restrict, trace = 1L, averaging_start = 400L)
...
state_rng_size_restrict_large <- take_xtra(
  state_rng_size_restrict_xtra, n_max = 1L, N_fw_n_bw = 25000L, 
  N_smooth = 100000L, N_first = 100000L, N_smooth_final = 2000L)
```
...
Plot the mean estimate of the random effect term

```{r show_mean_rng_est}
local({
  fit <- state_rng_size_restrict_large
  rel_size_var <- wz(dat$rel_size)
  Sigma <- fit$Q
  cat("Range\n")
  print(rel_size_range <- range(rel_size_var))
  
  meas <- get_cloud_means(fit)
  fixef <- fit$fixed_effects["wz(rel_size)"]
  
  x <- seq(rel_size_range[1], rel_size_range[2], length.out = 50)
  xi <- x + attr(rel_size_var, "mu")
  y <- seq_len(nrow(meas))
  yi <- make_ym_inv(unique(sort(dat$tstop)))
  yi <- as.integer(format(yi, "%Y")) + (as.integer(format(yi, "%m")) - 1L) / 12L
  stopifnot(length(yi) == length(y))
  xy <- expand.grid(rel_size = x, t = y)
  z <- sapply(split(xy, xy$t), function(xyi)
    meas[xyi$t[1], 1] + (meas[xyi$t[1], 2] + fixef)  * xyi$rel_size)
  
  n_plots <- 9L
  get_plot_device({
    for(i in 0:(n_plots - 1L))
      persp(xi, yi, z, ticktype = "detailed", theta = -45 + 360 * i / n_plots, 
            xlab = "\nLog relative market size", ylab = "\nYear", 
            zlab = "\nLog-hazard") 
  }, "rng-pred-mean", onefile = FALSE)
})
```
...
```{r sd_w_size_rng, cache = 1, dependson = "sd_rng"}
state_rng_size_derivs <- 
  get_approx_info_n_score(state_rng_size_restrict_large, n_rep = 200)
```

```{r show_sd_w_size_rng}
state_rng_size_derivs_final <- local({
  # setup matrix to apply restrictions 
  n_para_full <- length(state_rng_size_derivs[[1]]$score)
  n_fix <- length(state_rng_size_restrict_large$fixed_effects)
  K <- matrix(0, n_para_full - 2, n_para_full)
  diag(K[1:n_fix, 1:n_fix]) <- 1
  K[n_fix + 1L, n_fix + 1L] <-1
  diag(K[n_fix + 2:5, n_fix + 4:7]) <- 1
  rownames(K) <- names(state_rng_size_derivs[[1]]$score)[c(
    1:n_fix, n_fix + c(1, 4:7))]
  
  # look at score approximations
  scs <- sapply(state_rng_size_derivs, "[[", "score")
  scs <- K %*% scs
  scs <- apply(scs, 1, function(x) cumsum(x) / seq_along(x))
  matplot(scs, type = "l")
  cat("Scores:\n")
  ...
  obs_info <- lapply(state_rng_size_derivs, "[[", "obs_info")
  obs_info <- lapply(obs_info, function(x) tcrossprod(K %*% x, K))
  obs_info <- simplify2array(obs_info)
  ...
  sds <- suppressWarnings(
    sapply(obs_info, function(x) sqrt(diag(solve(x)))))
  print(sds[, (ncol(sds) - 20L):ncol(sds)], digits = 8)
  print(sds[, (ncol(sds) - 20L):ncol(sds)] / sds[, ncol(sds)], digits = 2)
  matplot(t(sds / sds[, ncol(sds)]), type = "l", col = "black", 
          ylim = c(0, 2))
  sdsx <- head(sds, -3)
  matplot(t(sdsx / sdsx[, ncol(sdsx)]), type = "l", col = "black", 
          ylim = c(0, 2))
  
  list(sds = sds, sd = sds[, ncol(sds)], obs_info = obs_info)
})

state_rng_size_derivs_final$sd
sqrt(diag(vcov(base_fit)))
```

</div>

Look at correlation in random effect term on linear predictor scale 
...
local({
  rel_size_var <- wz(dat$rel_size)
  Sigma <- state_rng_size_restrict_large$Q
  n <- 201
  cat("Range\n")
  print(rel_size_range <- range(rel_size_var))
  
  x <- seq(from = rel_size_range[1],to = rel_size_range[2], length = n)
  X <- split(rbind(1, x), rep(1:n, each = 2))
  . <- function(z1, z2)
      z1 %*% Sigma %*% z2 /sqrt(z1 %*% Sigma %*% z1 * z2 %*% Sigma %*% z2)
  corrs <- outer(X, X, function(x, y)
    vapply(seq_along(x), function(i) .(x[[i]], y[[i]]), numeric(1)))
  
  get_plot_device({
    lab <- "Relative log market size"
    contour(x, x, corrs, xlab = lab, ylab = lab)
    contour(x, x, corrs, levels = c(
      seq(-.4, .6, by = .2), seq(.7, .9, by = .1), seq(.92, .98, by = .02)), 
      xlab = lab, ylab = lab)
    
    x.mock <- x
    keep <- as.integer(seq(1, length(x), length.out = 10L)) 
    x.mock[keep] <- round(x[keep], 2)
    x.mock[-keep] <- NA
    heatmap(corrs, Rowv = NA,Colv = NA, scale = 'none', labRow = x.mock,
            labCol = x.mock, symm = T, xlab = lab, ylab = lab)
  }, "rng-lp-cor", onefile = FALSE)
})
```
...
local({
  ...
  . <- function(x)
    solve(x$obs_info[[length(x$obs_info)]])
  vcovs <- list(
    .(state_rng_inter_derivs_final), .(state_rng_size_derivs_final))
...
forecast_func <- function(
  ...
  stopifnot(is.factor(yh_i), is.integer(n_outer), is.integer(n_inner), 
  ...
  out <- parLapply(cl, 1:n_outer, function(...){
    ...
    # given phat we compute a constant monthly hazard over the period. This we 
    # can be used e.g., AUC computation so that we account for the length that 
    # we observe each firm
    haz <- log1p(-phat) / n_periods
    
    list(haz = if(is_glm) NULL else haz, id_out = id_out, Y_bar = Y_bar)
  })
  
  # gather default rates for individuals, compute the means, and gather the 
  # industry wide default rates
  haz <- if(!is_glm)
    rowMeans(sapply(out, "[[", "haz")) 
  else {
    phat <- tapply(phat, id, Reduce, f = function(x, y) x + (1 - x) * y, init = 0.)
    log1p(-phat) / n_periods
  }
  ...
  list(
    Y_bar = Y_bar, haz = haz, id_out = id_out, event_count = event_count)
}
...
forecast_summary <- lapply(seq_along(fitted_lvls), function(i){
  ...
  aucs <- if(!any(y_realized) || !any(!y_realized)){
    ...
    sapply(objs_i, function(x) c(auc(y_realized, x$haz)))
